...
if [ "$1" = "-h" ] || [ "$1" = "--help" ];
  then
  echo
  echo -e " Usage:\t\t\e[38;5;148msudo ./scripts/start_containers.sh <mysql-5.5>/<mariadb-5.5>/<mariadb-10.0>/<pgsql-9.1>/<pgsql-9.4><all>\e[39m "
  echo
  echo -e " Purpose:\tHelp start the testbot database containers and repos."
  echo
  echo -e "\t\t<mysql-5.5>/<mariadb-5.5>/<mariadb-10.0>/<pgsql-9.1>/<pgsql-9.4>: Defines the database type to start. "
